...
class ReportAnalysis(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    important_findings: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    cancer_type: Mapped[str | None] = mapped_column(String(255), nullable=True)
    cancer_stage: Mapped[str | None] = mapped_column(String(100), nullable=True)
    biomarkers: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    abnormal_values: Mapped[list | None] = mapped_column(JSONB, nullable=True)
    ...
    def __repr__(self) -> str:  # pragma: no cover
        return f"<ReportAnalysis id={self.id} report_id={self.report_id} status={self.status}>"
