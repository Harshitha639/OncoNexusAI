...
export const patientProfileSchema = z.object({
  ...
  phone_number: z.preprocess(emptyToUndefined, z.string().max(32).optional()),
  blood_group: z.preprocess(
    emptyToUndefined,
    z.enum(["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-", "unknown"]).optional(),
  ),
  height_cm: z.preprocess(
    emptyToUndefined,
    z.coerce.number().gt(0).le(300).optional(),
  ),
  weight_kg: z.preprocess(
    emptyToUndefined,
    z.coerce.number().gt(0).le(500).optional(),
  ),
  address: z.preprocess(emptyToUndefined, z.string().max(2000).optional()),
  emergency_contact_name: z.preprocess(emptyToUndefined, z.string().max(255).optional()),
  emergency_contact_phone: z.preprocess(emptyToUndefined, z.string().max(32).optional()),
  emergency_contact_relationship: z.preprocess(emptyToUndefined, z.string().max(100).optional()),
});
export type PatientProfileFormValues = z.infer<typeof patientProfileSchema>;

export const medicalProfileSchema = z.object({
  family_history: z.preprocess(emptyToUndefined, z.string().max(4000).optional()),
  allergies: z.preprocess(emptyToUndefined, z.string().max(2000).optional()),
  current_medications: z.preprocess(emptyToUndefined, z.string().max(2000).optional()),
...
export const appointmentSchema = z.object({
  doctor_name: z.string().trim().min(1, "Doctor name is required.").max(255),
  department: z.preprocess(emptyToUndefined, z.string().max(255).optional()),
  scheduled_at: z.string().min(1, "Please choose a date and time."),
  reason: z.preprocess(emptyToUndefined, z.string().max(2000).optional()),
});
